...
### H. Universal Voice Bootstrap Robustness & Non-Crashing Voice Review Gate
- **Issue**: In legacy `VoiceDesigner`, whenever a candidate exceeded the Whisper `wer_threshold` (0.20), candidate 0 was deleted from disk and a fatal `RuntimeError("Voice references failed transcript check: ...")` was thrown. This caused `POST /voices/bootstrap` to return HTTP 500, triggered 5 retries, and set the project status to `'error'`. This completely broke the pipeline and prevented the user from ever reaching Stage 3.5: Voice Review, making it impossible to listen to the audio, review warnings, pick alternate candidates, or regenerate.
- **Fix**:
  - **Universal Non-Crashing Policy**: In `voice/tts_server/voice_designer.py`, transcript validation failures **never crash the server** and **never delete the candidate audio**.
  - **Review Warning Recording**: Any candidate whose WER exceeds threshold is retained with its synthesized audio file intact, and a prominent warning is attached (`candidate.warnings.append(f"Reference transcript WER {wer:.2f} exceeded threshold ({self.wer_threshold:.2f}); manual audition and approval required in Voice Review.")`).
  - **Review Submission**: The bootstrap endpoint completes with HTTP 200 OK, `voice_cast.json` is safely persisted with all voices and warnings, and the pipeline parks cleanly at Stage 3.5: Voice Review (`waiting_for_review`), empowering the operator to audition, approve, swap, or regenerate candidates directly in the web UI.
  - **Unit Test Coverage**: Added `test_elevated_wer_is_retained_with_warning_and_does_not_crash` in `tests/test_voice_runtime_contracts.py` verifying that even at 45% WER, the candidate is retained with warnings and bootstrap succeeds with `status="success"`.
